...
export default function ExampleEditors() {
    return (
        <JsonEditorDependencies>
            <RenderExampleEditors schema={schema}
                startval={startval}/>
        </JsonEditorDependencies>
    )
}

function RenderExampleEditors({schema, startval}) {
    const editor1Data = {
        schema: schema,
        // optional
        startval: startval,
  ...
  // use useGetJsonEditorsData to get all config values from all editors
    const jsonEditorsData = useGetJsonEditorsData();
    function useSaveEditors() {
        console.log("editorsData:", jsonEditorsData)
    };
    const renderedEditors = (<><JsonEditor data={editor1Data} editorName={"Editor-1"}/> <JsonEditor data={editor1Data} editorName={"Editor-2"}/></>)
    return useMemo(() => (<>
